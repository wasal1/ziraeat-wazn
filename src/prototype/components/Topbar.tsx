...
export default function Topbar({ current, onToggle }: Props) {
  ...
  const title = t(`page.${current}.title`) !== `page.${current}.title`
    ? t(`page.${current}.title`)
    : current;
  const sub = t(`page.${current}.sub`) !== `page.${current}.sub`
    ? t(`page.${current}.sub`)
    : '';
  ...
  return (
    <header className="h-[58px] md:h-[62px] bg-white/90 backdrop-blur-md border-b border-gray-100/80 flex items-center px-3 md:px-5 gap-2 md:gap-3 sticky top-0 z-20 shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
      ...
      <div className="flex-1 min-w-0">
        <p className="font-bold text-gray-800 text-[14px] leading-tight truncate">{title}</p>
        {sub && <p className="text-[10px] text-gray-400 truncate hidden sm:block">{sub}</p>}
      </div>

      {/* Language toggle */}
      <button
        onClick={() => setLang(lang === 'ar' ? 'en' : 'ar')}
        className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-gray-50 border border-gray-200 text-gray-600 hover:border-green-300 hover:bg-green-50 hover:text-green-700 transition-all text-xs font-bold flex-shrink-0"
      >
        🌐 {lang === 'ar' ? 'EN' : 'ع'}
      </button>
