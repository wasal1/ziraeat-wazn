...
export default function Sidebar({ current, onNav, collapsed }: Props) {
  let lastGroup = '';

  return (
    <aside className={`fixed top-0 right-0 h-full z-30 flex flex-col bg-white border-l border-gray-100/80 shadow-lg transition-all duration-300 ${collapsed ? 'w-[68px]' : 'w-[260px]'}`}>

      {/* ─── Logo ─────────────────── */}
      <div className={`flex items-center gap-3 border-b border-gray-100/80 flex-shrink-0 ${collapsed ? 'px-3 py-4 justify-center' : 'px-5 py-4'}`}>
      ...
      <nav className="flex-1 overflow-y-auto py-3 px-2 scrollbar-thin scrollbar-thumb-gray-200">
        {NAV.map((item) => {
          ...
          return (
            <div key={item.id}>
              {showGroup && (
                <p className="text-[10px] text-gray-400 uppercase tracking-widest font-semibold px-3 pt-5 pb-1.5">
                  {item.group}
                </p>
              )}
              <button
              ...
              >
                ...
                {!collapsed && (
                  <>
                    <span className={`text-[13px] font-medium flex-1 text-right truncate ${isActive ? 'text-white' : ''}`}>
                      {item.label}
                    </span>
                    {!item.live && (
                      <span className="text-[9px] font-semibold px-1.5 py-0.5 rounded-full bg-gray-100 text-gray-400">
                        قريباً
                      </span>
                    )}
                    {isActive && (
      ...
      {/* ─── User Footer ──────────── */}
      <div className={`border-t border-gray-100/80 flex-shrink-0 ${collapsed ? 'p-2 flex justify-center' : 'px-4 py-3'}`}>
        {collapsed ? (
